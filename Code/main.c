...
#define MAX_EVENT_NUMBER 9999  //最大事件数
...
static int epollfd = 0;//------------------------内核事件表，epoll实例
...
/*信号处理函数（传入sigaction结构体使用），仅仅将信号值从管道写端写入，不处理信号对应的逻辑，缩短异步执行时间，减少对主程序的影响*/
...
int main(int argc, char *argv[])
{
    ...
    //使用epoll的步骤为socket(),setsockopt(),bind(),listen(),epoll_create(),epoll_ctl(),epoll_wait(),accept()

    /*=====================================socket()==============================================*/
    int listenfd = socket(PF_INET, SOCK_STREAM, 0);//---服务器用于监听客户端的连接请求的套接字
    assert(listenfd >= 0);
    
    /*------------创建可以用于下面bind()函数绑定的服务器端的套接字地址------------- */
    struct sockaddr_in server_address;
    bzero(&server_address, sizeof(server_address));//----将 server_address 的内存空间清零
    server_address.sin_family = AF_INET;//---------------设置地址协议族为IPV4
    server_address.sin_addr.s_addr = htonl(INADDR_ANY);//设置IP地址为任意IP地址
    server_address.sin_port = htons(port);//-------------设置端口号为9527
    /*====================================setsocketopt()=========================================*/
    int flag = 1;                           //-----------设置端口复用，可以在套接字关闭后立即重新使用之前绑定的地址
    setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, &flag, sizeof(flag));
    /*=======================================bind()=============================================*/
    int ret = 0;                            //-----------将套接字绑定到刚创建的服务器端的的地址，使其监听指定的 IP 地址和端口
    ret = bind(listenfd, (struct sockaddr *)&server_address, sizeof(server_address));
    assert(ret >= 0);
    /*======================================listen()============================================*/
    ret = listen(listenfd, 5);//------------------------设置可以同时等待处理的最大连接数为5
    assert(ret >= 0);

    //epollfd为内核事件表，events用于存储监听到的就绪事件
    epoll_event events[MAX_EVENT_NUMBER];//-------------用于存储就绪事件的数组
    /*====================================epoll_create()=========================================*/
    epollfd = epoll_create(5);//------------------------epoll实例（内核事件表）的文件描述符
    assert(epollfd != -1);
    /*=====================================epoll_ctl()===========================================*/
    addfd(epollfd, listenfd, false);//------------------addfd()里封装了epoll_ctl(),向内核事件表注册读事件
    http_conn::m_epollfd = epollfd;

    //创建管道套接字，用于处理信号，定时器相关
    ...
    addsig(SIGALRM, sig_handler, false);//--------------设置SIGALRM的信号处理函数为sig_handler（将信号值从管道写端写入），不设置SA_RESTART
    addsig(SIGTERM, sig_handler, false);//--------------设置SIGTERM的信号处理函数为sig_handler（将信号值从管道写端写入），不设置SA_RESTART  
